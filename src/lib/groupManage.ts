...
import {
  publishEvent, buildGroupMetadataEvent, buildGroupKeyBackupEvent,
} from './nostr'
import { generateGroupKey } from './groupCrypto'
import { serializeMembers, serializeGroupInvite, serializeGroupRekey, serializeGroupRemove } from './groupMembership'
import { sendGroupControl } from '../hooks/useNostrSubscriptions'
import { sendPrivate } from './privateSend'
...
export async function addGroupMember(group: Group, newMemberPubkey: string): Promise<void> {
  ...
  await sendPrivate(serializeGroupInvite(group.id, key, group.name, members), newMemberPubkey)
...
export async function removeGroupMember(group: Group, removePubkey: string): Promise<void> {
  ...
  for (const member of members) {
    ...
    await sendPrivate(serializeGroupRekey(group.id, newKey, group.name, members), member)
  }
  await sendPrivate(serializeGroupRemove(group.id), removePubkey)
